...
BEGIN
  ...
  LOOP
    ...
    IF v_emp.work_regime = '6x1' THEN
      v_candidate_day := (v_emp_idx % 6) + 1;
      IF v_candidate_day = 0 THEN v_candidate_day := 1; END IF;
      v_folga_day := v_candidate_day;
    ELSIF v_emp.work_regime = '5x2' THEN
      v_candidate_day := (v_emp_idx % 5) + 1;
      v_folga_day := v_candidate_day;
    ...
    VALUES (v_emp.id, v_emp.preferred_shift, v_emp.work_regime, v_folga_day, v_folga_day2, (v_folga_sunday IS NOT NULL), v_emp.responsibilities);
  END LOOP;
  ...
  FOR v_day IN 0..6 LOOP
    ...
    FOR v_emp IN SELECT es.*, e.name, e.color FROM _emp_sched es JOIN employees e ON e.id = es.employee_id LOOP
      ...
      IF v_shift_name IS NULL OR v_shift_name = 'flutuante' THEN
        v_shift_name := CASE WHEN v_emp_idx % 2 = 0 THEN 'abertura' ELSE 'fechamento' END;
      END IF;
      ...
      ELSE
        v_exit_time := CASE WHEN v_day = 0 THEN v_store.closing_exit_6x1 -- Simplificação, ideal seria checar regime
                            WHEN v_day = 6 THEN v_store.closing_exit_6x1 
                            ELSE v_store.closing_exit_6x1 END;
        IF v_emp.work_regime = '5x2' THEN
            v_exit_time := v_store.closing_exit_5x2;
        END IF;
