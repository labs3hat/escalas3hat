...
BEGIN
  ...
  LOOP
    -- Se não tem folga preferida OU se a folga for Sábado (5), atribuir uma baseada no index (Segunda a Sexta: 0 a 4)
    -- Isso garante que nem todos folguem no mesmo dia, que todos tenham folga e que NINGUÉM folgue no sábado.
    v_assigned_off_day := v_emp.preferred_day_off;
    IF v_assigned_off_day IS NULL OR v_assigned_off_day = 5 THEN
      v_assigned_off_day := (v_emp_index % 5);
    END IF;
    ...
    FOR v_day IN 0..6 LOOP
      ...
      -- Folga se for o Domingo escalado OU se for o dia de folga da semana (v_day 0=Seg, ..., 4=Sex)
      -- Importante: NUNCA dar folga no sábado (v_dow = 6 / v_day = 5)
      IF (v_dow = 0 AND v_fixed_sunday = v_current_date) OR 
         (v_day = v_assigned_off_day AND v_dow NOT IN (0, 6)) THEN
