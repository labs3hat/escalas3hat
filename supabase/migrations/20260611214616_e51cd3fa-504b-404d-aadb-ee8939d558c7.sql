...
BEGIN
  ...
  LOOP
    ...
    FOR v_day IN 0..6 LOOP
      ...
      -- Determinar horários de abertura e fechamento da loja para o dia específico
      IF v_day = 0 THEN -- Domingo
        v_entrada_real := v_store.opening_time_sunday;
        v_exit_time := v_store.closing_time_sunday;
      ELSIF v_day = 6 THEN -- Sábado
        v_entrada_real := v_store.opening_time_saturday;
        v_exit_time := v_store.closing_time_saturday;
      ELSE -- Dias de semana
        v_entrada_real := v_store.opening_time_weekday;
        v_exit_time := v_store.closing_time_weekday;
      END IF;

      -- Ajustar horários do funcionário baseados no turno
      -- (Simplificado: manhã entra na abertura, tarde sai no fechamento)
