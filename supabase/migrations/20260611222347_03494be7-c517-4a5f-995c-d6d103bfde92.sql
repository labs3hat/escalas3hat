...
BEGIN
  ...
  LOOP
    v_target_offs := CASE WHEN v_emp.work_regime = '5x2' THEN 2 ELSE 1 END;
    v_offs_count := 0;

    -- Verificar antecipadamente se este funcionário tem folga neste domingo (DOW 0)
    ...
    FOR v_day IN 0..6 LOOP
      ...
      ELSIF v_dow NOT IN (0, 6) AND v_offs_count < v_target_offs THEN
        -- Se domingo é folga e regime é 6x1, não damos mais folgas
        -- Se domingo é folga e regime é 5x2, ainda precisamos de 1 folga
        -- Se domingo NÃO é folga, precisamos de v_target_offs folgas
        
        -- Verificar se ainda precisamos de folgas considerando que o domingo PODE ser folga no futuro (fim do loop)
        -- Mas já calculamos v_sunday_is_off.
        
        IF (v_sunday_is_off AND v_target_offs = 1) THEN
           -- Já tem a folga do domingo garantida, não dar folga na semana
           NULL;
        ELSE
           -- Tentar atribuir folga no dia fixo/preferencial OU em qualquer dia disponível se estiver acabando a semana
           IF (v_dow = v_emp.fixed_day_off OR v_dow = v_emp.preferred_day_off OR v_dow >= 3) AND NOT v_is_responsible THEN
              -- Garantir que não daremos folgas demais
              IF v_offs_count < (v_target_offs - CASE WHEN v_sunday_is_off THEN 1 ELSE 0 END) THEN
                 v_slot_type_val := 'day_off';
                 v_offs_count := v_offs_count + 1;
              END IF;
           END IF;
  ...
  RETURN jsonb_build_object('success', true, 'slots_created', v_slots_created, 'schedule_id', v_schedule_id);
END;
$function$;