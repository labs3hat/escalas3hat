...
DECLARE
  ...
  v_break_duration      INT := 60;
  v_last_break_start    TIME;
BEGIN
  ...
  LOOP
    ...
    v_folga_day := v_emp.preferred_off_day;
    v_found2 := false;
    IF v_emp.work_regime = '5x1' THEN
    ...
    VALUES (v_emp.id, v_emp.preferred_shift, v_emp.work_regime, v_folga_day, v_folga_day2, v_found, v_emp.responsibilities);
  ...
  FOR v_day IN 1..7 LOOP
    ...
    LOOP
      IF (v_day = 7 AND v_emp.folga_sunday) OR 
         (v_emp.work_regime = '5x1' AND v_day = v_emp.folga_weekday) OR
         (v_emp.work_regime = '5x2' AND (v_day = v_emp.folga_weekday OR v_day = v_emp.folga_weekday2)) 
      THEN
      ...
      IF FOUND THEN
        ...
        -- Logic for FIFO break staggering:
        -- The first to arrive is the first to go on break.
        -- We stagger breaks by 30 minutes, starting from 3 hours after entry,
        -- but always ensuring the employee doesn't work more than 6 hours consecutively.
        
        v_break_start := v_entrada_real + INTERVAL '3 hours' + (v_break_offset * INTERVAL '30 minutes');
        
        -- If break would start after 5.5 hours of work, cap it to 5 hours to be safe
    ...
    -- Ensure minimum staff at opening and closing
    ...
    FOR v_si IN 0..1 LOOP
       ...
       IF v_working_today = 0 THEN
          -- Take the first available employee not on day off and force work slot
