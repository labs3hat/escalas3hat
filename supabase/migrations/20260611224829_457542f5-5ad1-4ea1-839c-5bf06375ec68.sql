  ...
  LOOP
    ...
    FOR v_day IN 0..6 LOOP
      ...
      ELSIF v_dow NOT IN (0, 6) AND v_offs_count < v_target_offs THEN
        -- Distribuição proporcional: Segunda(1) a Sexta(5)
        -- Usamos o índice do funcionário para garantir que nem todos folguem no mesmo dia
        v_calculated_off_day := (v_emp_index % 5) + 1; 

        IF (
          (v_emp.fixed_day_off > 0 AND v_dow = v_emp.fixed_day_off) OR 
          (v_dow = v_calculated_off_day)
      ...
      ELSE
        ...
        -- Definição de horários baseada no DOW e tipo de loja
        IF v_dow = 0 THEN 
        ...
        -- Ajuste de Turno (Simplificado para geração base)
        ...
        WHILE v_current_time < v_entrada_real + INTERVAL '9 hours' LOOP
          IF v_current_time >= v_interval_start AND v_current_time < v_interval_end THEN
            v_slot_type_val := 'interval';
          ELSE
            v_slot_type_val := 'work';
          END IF;
