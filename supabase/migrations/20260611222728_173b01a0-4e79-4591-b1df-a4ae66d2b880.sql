    ...
    FOR v_day IN 0..6 LOOP
      ...
      ELSIF v_dow NOT IN (0, 6) AND v_offs_count < v_target_offs THEN
        IF (v_sunday_is_off AND v_target_offs = 1) THEN
           NULL;
        ELSE
           -- Valor 0 na interface significa "Não/Sem folga fixa", logo ignoramos no DOW (que é Domingo=0)
           IF (
             (v_emp.fixed_day_off > 0 AND v_dow = v_emp.fixed_day_off) OR 
             (v_emp.preferred_day_off > 0 AND v_dow = v_emp.preferred_day_off) OR 
             (v_dow >= 3) -- Fallback para Quarta, Quinta ou Sexta
           ) AND NOT v_is_responsible THEN
      ...
      ELSE
        ...
        WHILE v_current_time < v_exit_time LOOP
          INSERT INTO schedule_slots (schedule_id, employee_id, day_of_week, slot_time, slot_type, updated_by)
          VALUES (v_schedule_id, v_emp.id, v_dow, v_current_time, 
            CASE WHEN v_current_time >= v_interval_start AND v_current_time < v_interval_end THEN 'interval'::slot_type
            ELSE 'work'::slot_type END, p_created_by);
          v_slots_created := v_slots_created + 1;
