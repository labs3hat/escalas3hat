...
BEGIN
  ...
  FOR v_day IN 0..6 LOOP
    ...
    v_min_opening := COALESCE(v_store.min_opening_staff, 1);
    v_min_closing := COALESCE(v_store.min_closing_staff, 2);

    SELECT COUNT(DISTINCT employee_id) INTO v_ab_count
    FROM schedule_slots
    WHERE schedule_id = v_schedule_id AND day_of_week = v_day AND slot_time = v_opening_ref AND slot_type = 'work';
    
    IF v_ab_count < v_min_opening THEN
      FOR v_fl_i IN 1..(v_min_opening - v_ab_count) LOOP
        INSERT INTO freelancer_slots (schedule_id, store_id, day_of_week, shift_name, rule_origin, start_time, end_time)
        VALUES (v_schedule_id, p_store_id, v_day, 'Abertura', 'R1', v_opening_ref - INTERVAL '1 hour', v_opening_ref + INTERVAL '7 hours 20 minutes');
        v_freelancers_created := v_freelancers_created + 1;
      END LOOP;
    END IF;

    SELECT COUNT(DISTINCT employee_id) INTO v_fc_count
    FROM schedule_slots
    WHERE schedule_id = v_schedule_id AND day_of_week = v_day AND slot_time = v_closing_ref AND slot_type = 'work';

    IF v_fc_count < v_min_closing THEN
      FOR v_fl_i IN 1..(v_min_closing - v_fc_count) LOOP
        INSERT INTO freelancer_slots (schedule_id, store_id, day_of_week, shift_name, rule_origin, start_time, end_time)
        VALUES (v_schedule_id, p_store_id, v_day, 'Fechamento', 'R2', v_closing_exit - INTERVAL '8 hours 20 minutes', v_closing_exit);
        v_freelancers_created := v_freelancers_created + 1;
      END LOOP;
    END IF;
