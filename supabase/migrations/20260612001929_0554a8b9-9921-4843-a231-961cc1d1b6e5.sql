CREATE OR REPLACE FUNCTION public.generate_base_schedule(p_store_id uuid, p_week_start date, p_created_by uuid DEFAULT NULL::uuid)
 ...
 SECURITY DEFINER
AS $function$
-- v20: restauração da v19 (sobrescrita indevidamente em 11/jun por versão legado)
-- Garantias: folga apenas seg-sex (formula % 5 + 1); INSERT em hours_bank; slots alinhados :00/:30
DECLARE
  v_schedule_id         UUID;
  v_store               RECORD;
  v_emp                 RECORD;
