CREATE OR REPLACE FUNCTION public.generate_base_schedule(p_store_id uuid, p_week_start date, p_created_by uuid DEFAULT NULL::uuid)
 ...
 SECURITY DEFINER
AS $function$
-- v17: alinha entrada ao grid de 30 min (fechamento 5x2 caía em :02/:32 e ficava invisivel na grade)
DECLARE
  v_schedule_id         UUID;
  v_store               RECORD;
  v_emp                 RECORD;
...
BEGIN
  ...
  FOR v_day IN 0..6 LOOP
    ...
    SELECT COUNT(*) INTO v_int_available FROM _emp_sched
    WHERE pref_shift NOT IN ('abertura', 'fechamento')
      AND NOT (folga_weekday = v_day AND v_day BETWEEN 1 AND 5)
      AND NOT (v_day = 0 AND folga_sunday)
      AND NOT (folga_weekday2 IS NOT NULL AND folga_weekday2 = v_day AND v_day BETWEEN 1 AND 6);
    ...
    LOOP
      ...
      IF v_emp.pref_shift NOT IN ('abertura', 'fechamento')
         AND v_fc_today < (CASE WHEN v_day IN (0,6)
                                THEN v_store.min_closing_weekend
                                ELSE v_store.min_closing_staff END)
         AND v_fc_today >= v_fc_available
         AND v_int_as_fc_today < (CASE WHEN v_day IN (0,6)
                                       THEN v_store.min_closing_weekend
                                       ELSE v_store.min_closing_staff END
                                   - v_fc_available)
      ...
      -- v17: alinhar a entrada a um bloco de 30 min para que os slots caiam
      -- sempre em :00 / :30 (a grade horária só renderiza esses minutos).
      -- Sem isto, o fechamento 5x2 (ex.: 22:20 - 9:48 = 12:32) gerava slots
      -- em :02/:32 que ficavam invisiveis na grade.
  ...
  FOR v_day IN 0..6 LOOP
    ...
    DECLARE
      ...
      v_min_fc INT := CASE WHEN v_day IN (0,6) THEN v_store.min_closing_weekend ELSE v_store.min_closing_staff END;
  ...
  FOR v_day IN 0..6 LOOP
    ...
    WHERE ss.schedule_id = v_schedule_id AND ss.day_of_week = v_day
      AND ss.slot_type = 'work'
      AND ss.slot_time >= v_closing_exit - INTERVAL '2 hours';
    IF COALESCE(v_fc_count,0) < (CASE WHEN v_day IN (0,6) THEN v_store.min_closing_weekend ELSE v_store.min_closing_staff END) THEN
      SELECT COUNT(*) INTO v_int_count FROM freelancer_slots
      WHERE schedule_id = v_schedule_id AND day_of_week = v_day
      ...
      v_violations := v_violations || jsonb_build_object(
        ...
        'day_of_week',v_day,'employee_ids',to_jsonb(ARRAY[]::text[]),
        'message','R2: '||COALESCE(v_fc_count,0)||'/'
