...
BEGIN
  ...
  FOR v_day IN 0..6 LOOP
    ...
    LOOP
      ...
      IF (v_break_start - v_entrada_real) > INTERVAL '6 hours' THEN
        v_break_start := v_entrada_real + INTERVAL '5 hours 30 minutes';
      ELSIF (v_break_start - v_entrada_real) < INTERVAL '1 hour' THEN
        v_break_start := v_entrada_real + INTERVAL '1 hour';
      END IF;

      v_break_end := v_break_start + INTERVAL '60 minutes';

      IF (v_saida_real - v_break_end) > INTERVAL '6 hours' THEN
        v_break_start := v_saida_real - INTERVAL '7 hours'; 
        v_break_end   := v_break_start + INTERVAL '60 minutes';
      END IF;

      v_break_start := TIME '00:00' + (FLOOR(EXTRACT(EPOCH FROM v_break_start) / 1800) * 1800) * INTERVAL '1 second';
