CREATE OR REPLACE FUNCTION public.generate_base_schedule(p_store_id uuid, p_week_start date, p_created_by uuid DEFAULT NULL::uuid)
 ...
 SECURITY DEFINER
AS $function$
-- v22: Corrigida distribuição de folgas (R17) para respeitar limites simultâneos globais
DECLARE
  v_schedule_id         UUID;
  v_store               RECORD;
  v_emp                 RECORD;
  ...
  v_global_folga_counts INT[] := ARRAY[0,0,0,0,0,0,0]; -- 0:Dom, 1:Seg, ... 6:Sáb
  v_max_sim_offs        INT;
  v_shift_name          TEXT;
  v_break_start         TIME;
  v_break_end           TIME;
...
BEGIN
    ...
    ELSE
      ...
      IF NOT v_found THEN
        FOR v_si IN 0..4 LOOP
          ...
          -- R17: Respeitar limite global de folgas simultâneas
    ...
    IF v_emp.work_regime = '5x2' THEN
      ...
      ELSE
        ...
        FOR v_si IN 0..4 LOOP
          ...
          -- R17: Limite para segunda folga também
          IF v_global_folga_counts[v_candidate_day] >= v_max_sim_offs AND v_si < 4 THEN CONTINUE; END IF;
  ...
  FOR v_day IN 0..6 LOOP
    ...
    -- intermediarios disponiveis para virar fechamento
    ...
    LOOP
      ...
      WHILE v_slot_time < v_entrada_real + v_jornada_bruta LOOP
    ...
    IF v_fc_working < v_min_required THEN
      ...
      FOR v_fl_i IN 1..v_gap LOOP
        INSERT INTO freelancer_slots (schedule_id, store_id, day_of_week, shift_name)
        VALUES (schedule_id, p_store_id, v_day, 'Fechamento');
        v_freelancers_created := v_freelancers_created + 1;
      END LOOP;
    END IF;
