...
BEGIN
  ...
  LOOP
    -- Mapeamento: NULL ou 0 = Sem preferência, 1 = Seg, ..., 5 = Sex, 6 = Sáb (NÃO PERMITIDO)
    -- Se for Sábado (6) ou sem preferência, atribui 1 a 5 (Seg a Sex)
    v_assigned_off_day := v_emp.preferred_day_off;
    IF v_assigned_off_day IS NULL OR v_assigned_off_day = 0 OR v_assigned_off_day = 6 THEN
      v_assigned_off_day := (v_emp_index % 5) + 1;
    END IF;
    ...
    FOR v_day IN 0..6 LOOP
      ...
      -- Folga se for o Domingo escalado (v_dow=0) OU se for o dia de folga da semana (1=Seg..5=Sex)
      -- v_day=0 é Segunda, logo (v_day+1)=1.
      -- v_dow=6 é Sábado. Nunca dar folga no sábado.
      IF (v_dow = 0 AND v_fixed_sunday = v_current_date) OR 
         ((v_day + 1) = v_assigned_off_day AND v_dow NOT IN (0, 6)) THEN
