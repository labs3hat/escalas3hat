...
DECLARE
  ...
  v_extra_sundays       INT;
BEGIN
  -- v28: carga semanal individual via employees.notes ("Xh semanais", coluna I da planilha).
  -- Fallback: weekly_hours da loja (colunas W/X aba LOJAS). Sem arredondamento de jornada.
  ...
  DELETE FROM hours_bank       WHERE schedule_id = v_schedule_id;
  ...
  FOR v_day IN 0..6 LOOP
    ...
    FOR v_emp IN
      SELECT e.*, es.pref_shift, es.folga_weekday, es.folga_weekday2,
             es.folga_sunday AS emp_folga_sun, es.notes AS emp_notes
      FROM employees e
    ...
    LOOP
      ...
      v_weekly_hours_emp := NULL;
      IF v_emp.emp_notes ~* '\d+\s*h\s*semanais' THEN
        v_weekly_hours_emp := (substring(v_emp.emp_notes from '(\d+)\s*[hH]\s*semanais'))::DECIMAL;
      END IF;
      
      IF v_weekly_hours_emp IS NULL OR v_weekly_hours_emp <= 0 OR v_weekly_hours_emp > 60 THEN
        IF v_emp.work_regime = '5x2' THEN
      ...
      INSERT INTO hours_bank (schedule_id, employee_id, week_start, scheduled_hours, extra_hours, updated_by)
      VALUES (
        v_schedule_id, 
        v_emp.id, 
        p_week_start,
