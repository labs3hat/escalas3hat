-- 1) Backfill de cores: distribui a paleta por loja para quem está com a cor padrão
WITH palette AS (
  SELECT ARRAY[
...
ranked AS (
  SELECT e.id,
         row_number() OVER (PARTITION BY e.store_id ORDER BY e.created_at, e.name) AS rn
  FROM employees e
)
UPDATE employees e
SET color = (SELECT colors[((r.rn - 1) % 12) + 1] FROM palette)
FROM ranked r
WHERE e.id = r.id
  AND (e.color IS NULL OR e.color = '#185FA5');

-- 2) Função de geração: v18 — fechamento mínimo de 2 quando >= 4 trabalhando no dia
CREATE OR REPLACE FUNCTION public.generate_base_schedule(p_store_id uuid, p_week_start date, p_created_by uuid DEFAULT NULL::uuid)
 ...
 SECURITY DEFINER
AS $function$
-- v18: regra geral — quando >= 4 funcionários trabalhando no dia, fechamento exige no minimo 2
DECLARE
  v_schedule_id         UUID;
  v_store               RECORD;
  v_emp                 RECORD;
...
BEGIN
  ...
  FOR v_day IN 0..6 LOOP
    ...
    -- v18: total de funcionarios trabalhando no dia e minimo de fechamento dinamico
  ...
  FOR v_day IN 0..6 LOOP
    ...
    WHERE ss.schedule_id = v_schedule_id AND ss.day_of_week = v_day
      AND ss.slot_type = 'work'
      AND ss.slot_time >= v_closing_exit - INTERVAL '2 hours';
    -- v18: minimo de fechamento sobe para 2 quando >= 4 trabalhando no dia
